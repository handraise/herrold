...
class EmailNotificationService {
  ...
  async sendTestResults(email, results, jobId) {
    ...
    // Extract sender name and reply-to address from EMAIL_FROM
    let senderName = 'Herrold Test Runner';
    let replyToAddress = 'noreply@herrold.com';
    
    if (process.env.EMAIL_FROM) {
      // Parse format like "Name <email@domain.com>"
      const fromMatch = process.env.EMAIL_FROM.match(/^(.+?)\s*<(.+)>$/);
      if (fromMatch) {
        senderName = fromMatch[1].trim();
        replyToAddress = fromMatch[2].trim();
      } else {
        // If just an email address, use it as reply-to
        replyToAddress = process.env.EMAIL_FROM;
      }
    }
    
    const mailOptions = {
      from: `${senderName} <${process.env.SMTP_USER}>`, // Gmail requires authenticated user
      replyTo: replyToAddress, // Where replies should go
      to: email,
      subject: subject,
      html: html,
